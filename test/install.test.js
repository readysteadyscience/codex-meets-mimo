...
test('one installer configures both sides without replacing other MiMo plugins', { skip: process.platform !== 'darwin' }, () => {
  ...
  try {
    ...
    assert.ok(existsSync(join(home, '.config', 'mimocode', 'skills', 'codex-meets-mimo', 'SKILL.md')));
    const installed = join(home, '.local', 'share', 'codex-meets-mimo');
    assert.ok(existsSync(join(installed, 'marketplace', 'plugins', 'codex-meets-mimo', 'assets', 'bridge.png')));
    const mcp = JSON.parse(readFileSync(join(installed, 'marketplace', 'plugins', 'codex-meets-mimo', '.mcp.json')));
    assert.ok(existsSync(mcp.mcpServers['codex-meets-mimo'].args[0]));
    const calls = readFileSync(join(temp, 'codex.log'), 'utf8');
    assert.match(calls, /plugin marketplace add/);
    assert.match(calls, /plugin add codex-meets-mimo@codex-meets-mimo-local/);
